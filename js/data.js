/* ===========================================================================
   ...
   Что заменить перед запуском (сейчас стоят заглушки):
     ...
     5. После любой правки BOOKS пересоберите страницы книг для поиска:
...
const STORE_CONFIG = {
  ...
  // Адрес сайта — для страниц книг и sitemap.xml. Если купите свой домен,
  // поменяйте здесь и перезапустите: node tools/build-pages.mjs
  siteUrl: 'https://xuz4ima-sketch.github.io/badr-magazin/',

  // Аналитика. Пусто — счётчик не подключается.
  // Яндекс Метрика: только номер счётчика, например '98765432'
  // (metrika.yandex.ru → Добавить счётчик).
  yandexMetrikaId: '',
  // Google Analytics: идентификатор вида 'G-XXXXXXXXXX'
  // (analytics.google.com → Администратор → Потоки данных).
  googleAnalyticsId: ''
};
